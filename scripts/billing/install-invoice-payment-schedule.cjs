...
write(
  "src/components/erp/billing/InvoicePaymentSchedule.tsx",
`"use client";
...
if (!form.includes("InvoicePaymentSchedule")) {
  form = form.replace(
`import {
...
if (!form.includes("data-invoice-payment-schedule")) {
  form = form.replace(
`      {isInvoiceEditForm ? (
...
save(
  historyPath,
  fixEncoding(read(historyPath))
);

console.log("");
console.log("Invoice payment schedule installed.");
console.log("Done.");