...
"@ | Set-Content ".\src\core\utils\formatFirestoreDate.ts" -Encoding UTF8

Write-Host "Helper global formatDisplayValue créé."
Write-Host ""
Write-Host "Maintenant cherche les affichages directs d'objets Firestore :"
Write-Host ""

Get-ChildItem .\src -Recurse -Include *.tsx |
Where-Object { $_.FullName -notmatch "\\.next\\|\\node_modules\\" } |
Select-String -Pattern "createdAt|updatedAt|date|Date|timestamp" |
ForEach-Object {
