...
const passName = "billing-model-b2-remove-unsupported-field-group";
...
source = source.replace(/\n\s*group:\s*"Origine métier",/g, "");

if (source === before) {
  ok("No unsupported group property found. Nothing changed.");
} else {
  fs.writeFileSync(target, source, "utf8");
  ok("Removed unsupported group property from facturesauto source fields.");
}
