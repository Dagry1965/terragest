...
const passName = "BILLING-MODEL-D-D1";

const scenarioKey = "D-D1";
const interventionId = "billing-dd1-intervention";
const line1Id = "billing-dd1-line-diagnostic";
const line2Id = "billing-dd1-line-main-oeuvre";

const reportPath = path.join(
  root,
  "docs",
  "audits",
  "BILLING-MODEL-D-D1-seed-invoice-line-test-scenario.md"
);
...
async function main() {
  ...
  if (!existingFactures.empty) {
    fail(`A facture already exists for ${interventionId}. Refusing to seed a polluted D-D1 scenario.`);
  }
  ...
  const interventionPayload = {
    ...
    description:
      "Scénario contrôlé D-D1 : intervention avec deux lignes validées, sans facture initiale.",
    clientId,
    vehiculeId,
    statut: "en_cours",
  ...
  let report = "# BILLING-MODEL-D-D1 — Seed scénario test facturation\n\n";
  ...
  report += "## Garde-fous\n\n";
  report += "- Le scénario porte `billingModelTestScenario = D-D1`.\n";
  ...
  report += "BILLING-MODEL-D-D2 : passer l'intervention `billing-dd1-intervention` à `terminee` via l'application, puis vérifier `facturesauto` et `lignesfactureauto`.\n";
