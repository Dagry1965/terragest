...
const passName = "BILLING-MODEL-D-D3";

const scenarioKey = "D-D3";
const interventionId = "billing-dd3-intervention";
const line1Id = "billing-dd3-line-diagnostic";
const line2Id = "billing-dd3-line-main-oeuvre";

const reportPath = path.join(
  root,
  "docs",
  "audits",
  "BILLING-MODEL-D-D3-seed-invoice-line-test-scenario.md"
);
...
async function main() {
  ...
  if (!existingFactures.empty) {
    fail(`A facture already exists for ${interventionId}. Refusing to seed a polluted D-D3 scenario.`);
  }
  ...
  const interventionPayload = {
    ...
    code: "BILL-D-D3-INTERVENTION",
    numeroIntervention: "BILL-D-D3-INTERVENTION",
    titre: "Test facturation lignes depuis intervention",
    description:
      "Scénario contrôlé D-D3 : intervention avec deux lignes validées, sans facture initiale.",
    clientId,
    vehiculeId,
    statut: "en_cours",
  ...
  const line1Payload = {
    ...lineBase,
    code: "BILL-D-D3-L1",
    designation: "Diagnostic électronique D-D3",
  ...
  const line2Payload = {
    ...lineBase,
    code: "BILL-D-D3-L2",
    designation: "Main d’œuvre atelier D-D3",
  ...
  let report = "# BILLING-MODEL-D-D3 — Seed scénario test facturation\n\n";
  ...
  report += "## Garde-fous\n\n";
  report += "- Le scénario porte `billingModelTestScenario = D-D3`.\n";
  ...
  report += "BILLING-MODEL-D-D3 : passer l'intervention `billing-dd3-intervention` à `terminee` via l'application, puis vérifier `facturesauto` et `lignesfactureauto`.\n";
