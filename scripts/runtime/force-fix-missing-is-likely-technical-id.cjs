...
if (!content.includes("function isLikelyTechnicalId(")) {
  const marker = `function relationTargetIsEmpty(
  value: unknown
): boolean {
  return (
    value === undefined ||
  ...
  if (!content.includes(marker)) {
    console.error("FAILED: relationTargetIsEmpty marker not found.");
    process.exit(1);
  }
...
console.log("OK: isLikelyTechnicalId inserted after relationTargetIsEmpty.");