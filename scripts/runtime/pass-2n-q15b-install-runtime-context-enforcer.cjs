...
const contextDir = path.join(ROOT, "src", "runtime", "context");

writeFile(
  path.join(contextDir, "RuntimeContextTypes.ts"),
...
writeFile(
  path.join(contextDir, "RuntimeContextEnforcer.ts"),
`import type {
  ERPModule,
...
console.log("");
console.log("[OK] PASS 2N-Q15B Runtime Context Enforcer Foundation installé.");
console.log("");
