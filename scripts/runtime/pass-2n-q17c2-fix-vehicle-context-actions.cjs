...
function findFunctionBlock(content, functionName) {
  ...
  if (start < 0) {
    throw new Error(`Fonction introuvable: ${functionName}`);
  }

  const firstBrace = content.indexOf("{", start);

  if (firstBrace < 0) {
    throw new Error(`Accolade introuvable: ${functionName}`);
  }
    ...
    if (char === "}") {
      ...
      if (depth === 0) {
        return {
          ...
          block: content.slice(start, index + 1),
        };
      }
    }
  }

  throw new Error(`Fin fonction introuvable: ${functionName}`);
}
...
function patchERPModuleTypes() {
  ...
  backup(file, "q17c2-child-prefill-types");
...
function patchRelatedRecordsPanel() {
  ...
  backup(file, "q17c2-child-prefill");

  let content = read(file);

  if (!content.includes("function getParentValueForPrefill")) {
    const found = findFunctionBlock(content, "buildCreateHref");
...
function buildCreateHref(
  ...
  if (!content.includes("parentRecord,\n        parentRecordId,")) {
    content = replaceOnce(
      content,
      `      buildCreateHref(
        child,
...
function patchVehiculesModule() {
  ...
  backup(file, "q17c2-vehicle-context-actions");
...
function main() {
  console.log("");
  console.log("[PASS] 2N-Q17C2 - Fix vehicle context actions");
