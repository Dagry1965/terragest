...
const bannerPath = "src/components/erp/context/ERPContextBanner.tsx";

write(bannerPath, `"use client";
...
backup(runtimePagePath, ".bak-q20f-a-context-banner");

let runtimePage =
  fs.readFileSync(p(runtimePagePath), "utf8");

if (!runtimePage.includes("@/components/erp/context/ERPContextBanner")) {
...
if (!runtimePage.includes("<ERPContextBanner")) {
  runtimePage = runtimePage.replace(
...
console.log("");
console.log("[Q20F_A_DONE] Runtime ERP context banner foundation installed.");
console.log("");
console.log("Next:");
