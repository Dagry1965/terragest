...
const TAG = "q22e2-create-generic-scheduling-planning-view";

const dir = "src/components/erp/scheduling";
const file = `${dir}/ERPSchedulingPlanningView.tsx`;
...
if (fs.existsSync(full(file))) {
  const backup = `${full(file)}.bak-${TAG}`;
  if (!fs.existsSync(backup)) {
    fs.copyFileSync(full(file), backup);
...
fs.writeFileSync(
  full(indexFile),
  `export * from "./ERPSchedulingPlanningView";\n`,
  "utf8"
);

console.log(`[WRITTEN] ${file}`);
console.log(`[WRITTEN] ${indexFile}`);
console.log("[Q22E2_DONE] Generic scheduling planning view created.");
console.log("");
console.log("Next:");
