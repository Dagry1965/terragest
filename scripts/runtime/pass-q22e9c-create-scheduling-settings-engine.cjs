...
if (!fs.existsSync(indexPath)) {
  fail("settings/index.ts introuvable. Q22E-9B doit être appliqué avant Q22E-9C.");
}

const engineContent = `import {
...
if (!indexContent.includes("RuntimeSchedulingSettingsEngine")) {
  fs.writeFileSync(
    indexPath,
  ...
  console.log(`[SKIP] ${path.relative(root, indexPath)} already exports engine`);
}

console.log(`
[Q22E9C_DONE] RuntimeSchedulingSettingsEngine créé.

Scope:
  - moteur de résolution config effective
  - validation minimale
  - pas de Firestore
  - pas d'UI
  - pas encore branché planning/guards
