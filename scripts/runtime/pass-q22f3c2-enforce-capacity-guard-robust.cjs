...
const TAG = "q22f3c2-enforce-capacity-guard-robust";
...
const start = content.indexOf(`  const conflict =`);
const end = content.indexOf(`  if (context.operation === "update") {`, start);

if (start === -1) {
  throw new Error("[MISSING] conflict block start");
}

if (end === -1) {
  throw new Error("[MISSING] update block marker after conflict block");
}
...
console.log("[Q22F3C2_DONE] scheduling capacity guard installed with robust replacement.");
console.log("");
console.log("Next:");
