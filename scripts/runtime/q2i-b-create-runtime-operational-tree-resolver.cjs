...
const resolver = `import type { ERPModule } from "@/runtime/modules/ERPModule";
import {
...
write(resolverRel, resolver);

let indexFile = read(indexRel);

if (!indexFile.includes("RuntimeOperationalTreeResolver")) {
  indexFile += `
...
`;
  ...
  write(indexRel, indexFile);
} else {
  console.log("[SKIP]", indexRel, "RuntimeOperationalTreeResolver already exported");
}

console.log("");
console.log("[DONE] Q2-I-B RuntimeOperationalTreeResolver foundation created.");
console.log("");
console.log("Next:");
