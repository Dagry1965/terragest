...
backup(files.resolver, ".bak-q2i-jb2-node-summary-contract");
...
`;
  ...
  console.log("[SKIP] Summary types already exist in resolver");
}
...
backup(files.treeView, ".bak-q2i-jb2-render-node-summary");

const helpers = `function getSummaryToneClassName(tone?: string): string {
  switch (tone) {
    case "success":
...
if (!treeView.includes("function getSummaryToneClassName(")) {
  ...
  const foundAnchor = anchors.find((anchor) => treeView.includes(anchor));

  if (!foundAnchor) {
    throw new Error("No safe helper anchor found in ERPOperationalTreeView.");
  }

  treeView = treeView.replace(foundAnchor, helpers + foundAnchor);
  console.log("[ADDED] Summary helpers before anchor:", foundAnchor);
} else {
...
if (!treeView.includes("node.summary?.badges")) {
  const sourceAnchor = `                {sourceSummary ? (
...
`;
  ...
  if (treeView.includes(sourceAnchor)) {
    treeView = treeView.replace(sourceAnchor, sourceAnchor + "\n" + summaryBlock);
    console.log("[ADDED] Node summary rendering after source summary");
  } else {
    const subtitleAnchor = `                {node.subtitle ? (
...
`;
    ...
    if (!treeView.includes(subtitleAnchor)) {
      throw new Error("No safe visual insertion anchor found for node summary block.");
    }

    treeView = treeView.replace(subtitleAnchor, subtitleAnchor + "\n" + summaryBlock);
    console.log("[ADDED] Node summary rendering after subtitle");
  }
} else {
...
console.log("[DONE] Q2-I-J-B2 generic operational tree node summary view added.");
