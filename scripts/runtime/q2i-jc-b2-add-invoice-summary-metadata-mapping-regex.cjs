...
backup(files.erpModule, ".bak-q2i-jc-b2-tree-summary-config");
...
backup(files.resolver, ".bak-q2i-jc-b2-summary-mapping");

if (!resolver.includes("function getRuntimeOperationalSummaryRecordValue(")) {
  const helper = `function getRuntimeOperationalSummaryRecordValue(
...
`;
  ...
  const anchors = [
    "function buildRuntimeOperationalTreeSource(",
    "function getRuntimeOperationalTreeLabel(",
    "export class RuntimeOperationalTreeResolver",
  ];

  const foundAnchor = anchors.find((anchor) => resolver.includes(anchor));

  if (!foundAnchor) {
    throw new Error("No safe resolver helper anchor found.");
  }

  resolver = resolver.replace(foundAnchor, helper + foundAnchor);
  console.log("[ADDED] buildRuntimeOperationalTreeSummary helper before:", foundAnchor);
} else {
  console.log("[SKIP] Summary mapping helper already exists");
}

if (!resolver.includes("summary: buildRuntimeOperationalTreeSummary")) {
  const sourcePropertyRegex = /(source:\s*buildRuntimeOperationalTreeSource\([^)]*\),)/;

  if (!sourcePropertyRegex.test(resolver)) {
    throw new Error("Resolver node source property anchor not found.");
  }
...
backup(files.factures, ".bak-q2i-jc-b2-invoice-summary-config");
...
console.log("[DONE] Q2-I-J-C-B2 invoice summary metadata mapping added.");
