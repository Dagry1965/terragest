...
const PAYMENT_ID = "q2lc-b2-clean-partial-payment-001";
...
async function recomputeInvoice(factureRef, facture) {
  ...
  await factureRef.update({
    ...
    updatedBy: "q2lc-b2-clean-payment",
  });
...
async function main() {
  console.log("[Q2-L-C-B2] Create clean partial payment in tenant scope");

  const factureRef = db.collection("facturesauto").doc(FACTURE_ID);
  const factureSnap = await factureRef.get();

  if (!factureSnap.exists) {
    throw new Error(`[Q2-L-C-B2] Missing facture ${FACTURE_ID}`);
  }
  ...
  if (
    ...
    throw new Error(
      `[Q2-L-C-B2] Facture tenant/workspace mismatch: ${facture.tenantId}/${facture.workspace}`
    );
  }
  ...
  if (paymentSnap.exists) {
    ...
    if (
      ...
      console.log("[SKIP] Clean payment already exists", PAYMENT_ID);
      const recompute = await recomputeInvoice(factureRef, facture);
      console.log("[RECOMPUTED]", recompute);
      console.log("[Q2-L-C-B2] OK");
      return;
    }

    throw new Error(
      `[Q2-L-C-B2] Payment id already exists with incompatible data: ${PAYMENT_ID}`
    );
  }
  ...
  await paymentRef.set({
    ...
    numeroRecu: "REC-Q2-L-C-B2-001",
    montant: 20000,
    datePaiement: "2026-06-07",
    modePaiement: "mobile_money",
    referenceTransaction: "Q2-L-C-B2-PARTIAL",
    statut: "valide",

    notes: "Test Q2-L-C-B2 encaissement partiel propre dans le bon tenant.",
    createdAt: new Date().toISOString(),
    createdBy: "q2lc-b2-clean-payment",
    updatedAt: new Date().toISOString(),
    updatedBy: "q2lc-b2-clean-payment",
  });
  ...
  if (
    recompute.montantPaye !== 20000 ||
    recompute.resteAPayer !== 39000 ||
    recompute.statutPaiement !== "partiel"
  ) {
    throw new Error(
      `[Q2-L-C-B2] Unexpected recompute result: ${JSON.stringify(recompute)}`
    );
  }
...
main().catch((error) => {
  console.error("[Q2-L-C-B2][ERROR]", error);
  process.exit(1);
});